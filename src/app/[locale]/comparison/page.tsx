...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      ...
      languages: {
        'en': `${SITE_URL}/comparison`,
        'ko': `${SITE_URL}/ko/comparison`,
        'ja': `${SITE_URL}/ja/comparison`,
        'de': `${SITE_URL}/de/comparison`,
        'x-default': `${SITE_URL}/comparison`,
      },
    },
  };
}
...
export default async function Page({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const allContent = await getAllContent('comparison', validLocale);
  const articles = allContent.map(item => ({ slug: item.slug, metadata: item.metadata }));

  return <CategoryPage catKey="comparison" articles={articles} />;
}