...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      ...
      languages: {
        'en': `${SITE_URL}/entities`,
        'ko': `${SITE_URL}/ko/entities`,
        'ja': `${SITE_URL}/ja/entities`,
        'de': `${SITE_URL}/de/entities`,
        'x-default': `${SITE_URL}/entities`,
      },
    },
  };
}
...
export default async function Page({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const allContent = await getAllContent('entities', validLocale);
  const articles = allContent.map(item => ({ slug: item.slug, metadata: item.metadata }));

  return <CategoryPage catKey="entities" articles={articles} />;
}